# Demo Video

A 90-second screen recording of the system is planned for Day 16 (blog
post + LinkedIn share).

When recorded, the video will cover:

| Time | Scene |
|------|-------|
| 0:00 - 0:10 | GitHub repo README hero |
| 0:10 - 0:30 | Streamlit: ask "What is an asset?" → verified answer with citation |
| 0:30 - 0:50 | Streamlit: ask "What is the capital of France?" → honest refusal |
| 0:50 - 1:10 | Langfuse: open latest trace, show retrieval + generation spans + latency |
| 1:10 - 1:30 | GitHub Actions: green CI pipeline + red blocked-PR demo |
